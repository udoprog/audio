...
unsafe impl<T> Send for Channel<'_, T> where T: Sync {}
unsafe impl<T> Sync for Channel<'_, T> where T: Sync {}
...
impl<T> cmp::Eq for Channel<'_, T> where T: cmp::Eq {}
...
// Safety: the iterator is simply a container of references to T's, any
// Send/Sync properties are inherited.
unsafe impl<T> Send for ChannelIter<'_, T> where T: Sync {}
unsafe impl<T> Sync for ChannelIter<'_, T> where T: Sync {}
...
pub(crate) struct RawChannelRef<T: ?Sized> {
...
// Note: can't auto impl because `T`.
impl<T: ?Sized> Clone for RawChannelRef<T> {
    fn clone(&self) -> Self {
        *self
    }
}
impl<T: ?Sized> Copy for RawChannelRef<T> {}

pub(crate) struct RawChannelMut<T: ?Sized> {
...
// Note: can't auto impl because `T`.
impl<T: ?Sized> Clone for RawChannelMut<T> {
    fn clone(&self) -> Self {
        *self
    }
}
impl<T: ?Sized> Copy for RawChannelMut<T> {}
...
// Send/Sync properties are inherited.
unsafe impl<T> Send for ChannelMut<'_, T> where T: Send {}
unsafe impl<T> Sync for ChannelMut<'_, T> where T: Sync {}
...
impl<'a, T> ChannelMut<'a, T> {
    ...
    /// ```rust,compile_fail
    /// struct Foo<'a> {
    ///     left: audio::interleaved::ChannelMut<'a, f32>,
    /// }
    ///
    /// impl<'a> Foo<'a> {
...
impl<T> cmp::Eq for ChannelMut<'_, T> where T: cmp::Eq {}
...
// Safety: the iterator is simply a container of references to T's, any
// Send/Sync properties are inherited.
unsafe impl<T> Send for ChannelIterMut<'_, T> where T: Send {}
unsafe impl<T> Sync for ChannelIterMut<'_, T> where T: Sync {}
